...
GEOJ_TO_SHAPETYPE = {\
    ...
    "PointZ":POINTZ,
    "LineStringZ":POLYLINEZ,
    "PolygonZ":POLYGONZ,
    "MultiPointZ":MULTIPOINTZ,
    "PointM":POINTM,
    "LineStringM":POLYLINEM,
    "PolygonM":POLYGONM,
...
class Reader:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fromgeojson = None
        # See if a geojson dictionary was passed as an argument
        if len(args) > 0:
            if is_dict(args[0]):
                if args[0]["type"] != "FeatureCollection":
                    print "error: for pyshp to read a GeoJson dictionary file object it must of a FeatureCollection type."
                    return
                self.fromgeojson = args[0]
                return
    ...
    def __geojshape(self, record):
        geoj = self.fromgeojson
        record.shapeType = GEOJ_TO_SHAPETYPE[geoj["type"]]
        #record.bbox = []
        if geoj["type"] == "Point":
            record.points = geoj["coordinates"]
            record.parts = [0]
        elif geoj["type"] in ("MultiPoint","LineString","Polygon"):
            record.points = geoj["coordinates"]
            record.parts = [0]
    ...
    def __shape(self, numpyspeed=False):
        ...
        # if reading geojson then redirect to different shaperead function and do not proceed inside __shape
        if self.fromgeojson:
            emptyrecord = _Shape()
            record = __geojshape(emptyrecord)
            return record
        f = self.__getFileObj(self.shp)
        fread = f.read
        record = _Shape()
        nParts = nPoints = zmin = zmax = mmin = mmax = None
        (recNum, recLength) = unpack(">2i", fread(8))
        # Determine the start of the next record
        next = f.tell() + (2 * recLength)
        shapeType = unpack("<i", fread(4))[0]
        ...
        # All shape types capable of having a bounding box
            ...
            record.bbox = _Array('d', unpack("<4d", fread(32)))
        # Shape types with parts
        if shapeType in (3,5,13,15,23,25,31):
            nParts = unpack("<i", fread(4))[0]
        # Shape types with points
        if shapeType in (3,5,8,13,15,23,25,31):
            nPoints = unpack("<i", fread(4))[0]
        # Read parts
        if nParts:
            if numpyspeed:
        ...
        if shapeType in (13,15,18,31):
            ...
            record.z = _Array('d', unpack("<%sd" % nPoints, fread(nPoints * 8)))
        ...
        if shapeType in (13,15,18,23,25,28,31) and not 0.0 in self.measure:
            (mmin, mmax) = unpack("<2d", fread(16))
            # Measure values less than -10e38 are nodata values according to the spec
            record.m = []
            for m in _Array('d', unpack("<%sd" % nPoints, fread(nPoints * 8))):
                if m > -10e38:
                    record.m.append(m)
                else:
        ...
        if shapeType in (1,11,21):
            record.points = [_Array('d', unpack("<2d", fread(16)))]
        # Read a single Z value
        if shapeType == 11:
            record.z = unpack("<d", fread(8))
        # Read a single M value
        if shapeType in (11,21):
            record.m = unpack("<d", fread(8))
