...
#
unpack_big_i2 = struct.Struct(">2i").unpack
unpack_small_i = struct.Struct("<i").unpack
unpack_small_d = struct.Struct("<d").unpack
unpack_small_2d = struct.Struct("<2d").unpack
...
class Reader:
    ...
    def __shape(self, iterpoints=False, numpyspeed=False):
        ...
        (recNum, recLength) = unpack_big_i2(fread(8))
        # Determine the start of the next record
        next = f.tell() + (2 * recLength)
        shapeType = unpack_small_i(fread(4))[0]
        ...
        # All shape types capable of having a bounding box
            ...
            record.bbox = unpack("<4d", fread(32))
        # Shape types with parts
        if shapeType in (3,5,13,15,23,25,31):
            nParts = unpack_small_i(fread(4))[0]
        # Shape types with points
        if shapeType in (3,5,8,13,15,23,25,31):
            nPoints = unpack_small_i(fread(4))[0]
        # Read parts
        if nParts:
            record.parts = numpy.fromfile(f, numpy.dtype('<%si' % nParts), 1)
        # Read part types for Multipatch - 31
        if shapeType == 31:
            record.partTypes = unpack("<%si" % nParts, fread(nParts * 4))
        # Read points - produces a list of [x,y] values
        if nPoints:
            if numpyspeed: #numpyarray
                #print len(numpy.fromfile(f, numpy.dtype('<2d'), nPoints)), nPoints
                record.points = numpy.fromfile(f, numpy.dtype('<2d'), nPoints) # NUMPY VERSION
            else: #tuple of listiteration
                record.points = ([unpack_small_2d(fread(16)) for p in xrange(nPoints)])
            if iterpoints: #decides if put result into generator
                record.points = (each for each in record.points) #FASTEST NORMAL,RETURN AS TUPLE
                #print unpack_small_2d(fread(16))
                #arr = array.array("f")
                #for each in (arr.fromfile(f,16) for p in xrange(nPoints*2)):
                #    pass
                #record.points = arr
                #record.points = numpy.array([each for each in (unpack_small_2d(fread(16)) for p in xrange(nPoints))], dtype=object)
                #print str(record.points)[:100]
                
        # Read z extremes and values
        if shapeType in (13,15,18,31):
            (zmin, zmax) = unpack_small_2d(fread(16))
            record.z = unpack("<%sd" % nPoints, fread(nPoints * 8))
        ...
        if shapeType in (13,15,18,23,25,28,31) and not 0.0 in self.measure:
            (mmin, mmax) = unpack_small_2d(fread(16))
            # Measure values less than -10e38 are nodata values according to the spec
            record.m = []
            for m in unpack("<%sd" % nPoints, fread(nPoints * 8)):
                if m > -10e38:
                    record.m.append(m)
                else:
        ...
        if shapeType in (1,11,21):
            record.points = [unpack_small_2d(fread(16))]
        # Read a single Z value
        if shapeType == 11:
            record.z = unpack_small_d(fread(8))
        # Read a single M value
        if shapeType in (11,21):
            record.m = unpack_small_d(fread(8))
    ...
    def iterShapes(self, iterpoints=False, numpyspeed=False):
        ...
        shp.seek(100)
        while shp.tell() < self.shpLength:
            yield self.__shape(iterpoints=iterpoints, numpyspeed=numpyspeed)    
